...
class _Fixture:
    """Build a minimal ``<parent>/<repo>`` layout with formal task worktrees."""

    def __init__(self, temp: str):
        self.parent = Path(temp)
        self.repo = self.parent / "repo"  # main workspace replica of root.name
        self.repo.mkdir()

    def worktree_dir(self, task_id: str) -> Path:
        return self.parent / ".devflow-worktrees" / self.repo.name / task_id
...
class PlanPublishTest(unittest.TestCase):
    def test_create_skip_conflict_actions(self):
        ...
        self.assertEqual(mapping["prd.md"], "docs/tasks/t-9/prd-t-nine.md")
...
class PublishTest(unittest.TestCase):
    def test_publish_prd_under_semantic_name_and_others_fixed(self):
        ...
        result = ap.publish(f.repo, wt)
        self.assertEqual(result["errors"], [])
        self.assertEqual(result["conflicts"], [])
        target = f.repo / "docs" / "tasks" / "t-10"
    ...
    def test_two_tasks_same_prd_name_do_not_overwrite(self):
        ...
        ap.publish(f.repo, wt1)
        ap.publish(f.repo, wt2)

        one = f.repo / "docs" / "tasks" / "task-one" / "prd-task-one.md"
        two = f.repo / "docs" / "tasks" / "task-two" / "prd-task-two.md"
...
class ReadmeIndexTest(unittest.TestCase):
    def test_readme_contains_metadata_and_semantic_prd(self):
        ...
        ap.publish(f.repo, wt)
        readme = (f.repo / "docs" / "tasks" / "t-13" / "README.md").read_text(
            encoding="utf-8")
    ...
    def test_acceptance_report_and_scenarios_get_distinct_keys(self):
        ...
        ap.publish(f.repo, wt)
        readme = (f.repo / "docs" / "tasks" / "t-18" / "README.md").read_text(
            encoding="utf-8")
        ...
        self.assertIn('published: "docs/tasks/t-18/acceptance-report.md"', content)
        self.assertIn('published: "docs/tasks/t-18/acceptance-scenarios.md"', content)
...
class TaskYamlRefUpdateTest(unittest.TestCase):
    def test_update_writes_dual_path_refs(self):
        ...
        actions, mapping = ap.plan_publish(wt, f.repo / "docs" / "tasks" / "t-14")
        ap.update_task_artifact_refs(wt, "t-14", "t-fourteen", mapping)

        content = (wt / ".devflow" / "task.yaml").read_text(encoding="utf-8")
        self.assertIn('worktree: ".devflow/prd.md"', content)
        self.assertIn('published: "docs/tasks/t-14/prd-t-fourteen.md"', content)
    ...
    def test_update_preserves_delivery_field_and_does_not_touch_root(self):
        ...
        actions, mapping = ap.plan_publish(wt, f.repo / "docs" / "tasks" / "t-15")
...
class CliTest(unittest.TestCase):
    def test_cli_dry_run_outputs_plan_json(self):
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            out = ap.main(["publish", "--root", str(f.repo), "--task", "t-17",
                           "--dry-run"])
    ...
    def test_cli_conflict_exits_nonzero(self):
        ...
        target = f.repo / "docs" / "tasks" / "t-18"
        target.mkdir(parents=True)
        (target / "prd-t-eighteen.md").write_text("old", encoding="utf-8")

        with contextlib.redirect_stdout(io.StringIO()):
            out = ap.main(["publish", "--root", str(f.repo), "--task", "t-18"])
        self.assertEqual(out, 1)

    def test_cli_missing_root_exits_nonzero(self):
        f = _Fixture(tempfile.mkdtemp())
        with contextlib.redirect_stdout(io.StringIO()):
            out = ap.main(["publish", "--root", str(f.parent / "nope"), "--all-tasks"])
        self.assertEqual(out, 1)

    def test_cli_missing_discovery_exits_nonzero(self):
        f = _Fixture(tempfile.mkdtemp())
        with contextlib.redirect_stdout(io.StringIO()):
            out = ap.main(["publish", "--root", str(f.repo), "--task", "missing"])
        self.assertEqual(out, 1)
