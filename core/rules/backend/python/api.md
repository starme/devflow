---
description: Python API 设计规范（RPC 风格）
paths: ["**/*.py"]
alwaysApply: false
---
...
适用于 Python 后端服务的 HTTP API。RPC 风格，与具体框架无关（FastAPI / Flask / Django 均适用）。框架实现细节（路由装饰器、依赖注入写法、OpenAPI 注解）见项目级框架规范。
...
* **Step 2**：确认后生成完整路由定义、Pydantic 请求/响应模型、Handler 骨架
...
* 请求模型：`XxxReq`，响应模型：`XxxResp`，均继承 `pydantic.BaseModel`
* 一一对应，禁止复用请求/响应模型
* Handler 函数名与 action 名对应
...
* 统一 **camelCase**：`userId`、`createdAt`（与 URL 的 kebab-case 独立约定；Pydantic v2 用 `alias` 映射）
...
* 业务错误码集中定义在 `error_codes`（或等效的常量模块）
...
class BizError(Exception):
    """业务异常基类，子类定义 code 与 http_status。"""
    code: int
    message: str
    http_status: int = 400
...
框架层用全局异常处理器捕获 `BizError` 并按信封格式返回。
...
* 时间统一用 `datetime`（aware，带时区），序列化时格式化；禁止用裸字符串拼时间
...
接口文档中必须显式标注认证要求（FastAPI 用 `Depends`、Django/Flask 用装饰器或中间件），公开接口与鉴权接口分开声明，禁止混在同一分组。
