...
它会先识别仓库类型，而不是假设所有项目都是前后端应用；同时通过独立 task worktree、专职 Agent 和明确的安全门禁协调整个过程。

```mermaid
flowchart LR
    A["需求或 Bug"] --> M["DevFlow Manager<br/>分类 · 规划 · 调度"]
    M --> W["独立 task<br/>分支 + worktree"]
    W --> T["实现 + 测试"]
    T --> H["人工 Gate"]
    H --> P["提交 · 推送 · PR"]
    P -.-> R["评审与合并"]
...
- **安全隔离**——每个功能或 Bug 修复都有独立分支和 worktree。
...
使用 `/devflow status` 查看进度，使用 `/devflow next --task <task-id>` 恢复中断的任务。

## 它如何工作

1. **分类**——识别仓库类型，选择合适的工作流。
2. **澄清与规划**——必要时由产品和架构阶段产出 PRD、范围和实现方案。
3. **隔离与实现**——任务运行在独立分支和 worktree 中，Agent 遵守明确边界。
